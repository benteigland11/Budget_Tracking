...
def get_monthly_financial_summary(year, month, focused_main_category_id=None):
    db = get_db()
    category_details_query = """
        SELECT c.id as category_id, c.name as category_name, c.parent_id,
               p.name as parent_category_name, c.financial_goal_type,
               COALESCE(b.budgeted_amount, 0) as budgeted_amount,
               COALESCE(a.actual_amount, 0) as actual_amount
        FROM categories c
        LEFT JOIN categories p ON c.parent_id = p.id
        LEFT JOIN (
            SELECT category_id, SUM(amount) as actual_amount FROM transactions
            WHERE strftime('%Y', date) = ? AND strftime('%m', date) = ? AND type = 'expense' GROUP BY category_id
    ...
    """
    all_category_data = db.execute(category_details_query, (str(year), f"{month:02d}", year, month)).fetchall()
    summary_table_data = []
    expected_vs_actual_chart = {'labels': [], 'budgeted_data': [], 'actual_data': [], 'category_ids_for_drilldown': []}
    nws_actual_chart = {'labels': ['Needs', 'Wants', 'Savings/Investments', 'Unclassified'], 'data': [0, 0, 0, 0]}
    nws_budgeted_chart = {'labels': ['Needs', 'Wants', 'Savings/Investments', 'Unclassified'], 'data': [0, 0, 0, 0]}
    ...
    if focused_main_category_id:
        ...
        for cat_data in all_category_data:
            if cat_data['category_id'] == focused_main_category_id or cat_data['parent_id'] == focused_main_category_id:
                if cat_data['budgeted_amount'] > 0 or cat_data['actual_amount'] > 0:
                    full_name = cat_data['category_name']
                    if cat_data['category_id'] == focused_main_category_id and cat_data['parent_id'] is None:
                         full_name = f"{cat_data['category_name']} (Direct)"
                    summary_table_data.append({"name": full_name, "type": cat_data['financial_goal_type'], "budgeted": cat_data['budgeted_amount'], "actual": cat_data['actual_amount'], "variance": cat_data['budgeted_amount'] - cat_data['actual_amount']})
        ...
        for cat_data in all_category_data:
            ...
            if main_id_to_aggregate not in main_category_summary:
                ...
                main_cat_name = main_cat_name_row['name'] if main_cat_name_row else "Unknown"
                main_category_summary[main_id_to_aggregate] = {'name': main_cat_name, 'budgeted': 0, 'actual': 0, 'id': main_id_to_aggregate, 'nws_budgeted': [0,0,0,0], 'nws_actual': [0,0,0,0]}
        ...
        for main_id in sorted_main_cat_ids:
            ...
            if summary['budgeted'] > 0 or summary['actual'] > 0:
                summary_table_data.append({"name": summary['name'], "type": "Main", "budgeted": summary['budgeted'], "actual": summary['actual'], "variance": summary['budgeted'] - summary['actual']})
                ...
                for i in range(4): nws_budgeted_chart['data'][i] += summary['nws_budgeted'][i]; nws_actual_chart['data'][i] += summary['nws_actual'][i]
    return {"summary_table_data": summary_table_data, "expected_vs_actual_chart": expected_vs_actual_chart, "nws_actual_chart": nws_actual_chart, "nws_budgeted_chart": nws_budgeted_chart, "current_chart_title_suffix": current_chart_title_suffix, "focused_main_category_id": focused_main_category_id, "focused_main_category_name": focused_main_category_name}
...
def get_budget_goals_for_planning_ui(year, month):
    ...
    for main_cat in all_categories_structured:
        main_cat['budgeted_amount'] = budget_goals_map.get(main_cat['id'], 0.0)
