...
def get_financial_summary(year, month=None, period_type='monthly', focused_main_category_id=None):
    ...
    actuals_subquery_conditions_income = "strftime('%Y', date) = ? AND type = 'income'" # For income total
    budget_subquery_conditions = "year = ?"
    
    query_params_actuals_expense = [str(year)]
    query_params_actuals_income = [str(year)] # For income total
    query_params_budget = [year]

    if period_type == 'monthly':
        if month is None:
            # This should ideally not happen if logic in routes is correct
            current_app.logger.error("Month is None for monthly period_type in get_financial_summary. Defaulting to current month might be needed or raise error.")
            # For now, let's proceed, but this indicates a potential logic flow issue upstream.
            # Consider raising ValueError("Month must be provided for monthly summary.")
            # Or, if defaulting is acceptable: month = datetime.datetime.now().month 
            # For safety, if month is None here, we might not be able to form a valid query.
            # Let's assume month will be valid for now based on route logic.
            pass
        ...
        query_params_actuals_expense.append(f"{month:02d}")
        query_params_actuals_income.append(f"{month:02d}")
        query_params_budget.append(month)

    # MODIFICATION: Calculate period totals
    ...
    # For category_details_query, we need to combine params for actuals (expenses) and budget
    # The number of params for actuals_subquery_conditions_expense and budget_subquery_conditions must match the '?'
    ...
    if focused_main_category_id:
        # ... (rest of focused_main_category_id logic - no changes needed here) ...
        ...
        # ... (rest of main categories overview logic - no changes needed here) ...
    ...
    return {
        ...
        # MODIFICATION: Add new period totals to the return dictionary
...
def get_budget_goals_for_planning_ui(year, month):
    # ... (This function remains unchanged from db_helpers_py_v3) ...
    db = get_db()
    ...
    for main_cat in all_categories_structured:
        ...
        main_cat['budgeted_amount_from_subs'] = 0.0 
